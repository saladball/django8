...
#class 폼클래스명(From) : HTML에서 사용할 커스텀 폼을 정의

from django.forms.models import ModelForm
#from . import models #현재 폴더(.)에서 models 파일을 임포트
from .models import * #현재 폴더에 있는 models 파일에서 전체(*)을 임포트
...
class QuestionForm(ModelForm):
    class Meta: #Meta클래스 정의를 통해 모델 클래스에 관한 정보를 입력
        ...
        #         모델클래스의 변수명을 튜플형태로 입력
        #exclude : 모텔클래스의 변수 정 명시한 변수명을 제외한 나머지 변수들을
        #          클라이언트가 작성할 수 있도록 제공
        model = Question #해당 클래스가 Question 모델클래스와 연동됨을 명시
        #사용자가 question_text 변수에 들어갈 값만 입력할 수 있도록 <input>태그
        #가 생성됨
        #fields = ['question_text']
        #사용자가 pub_date변수 외에 변수들을 입력할 수 있도록<input>태그가 생성됨
        exclude = ['pub_date', 'author']

#선택지 문구, 어떤질문의 선택지인지 선택 할수있는 입력양식
#Choice 모델클래스를 참고
